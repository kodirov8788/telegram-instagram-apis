...
describe('DatabaseSecretProvider', () => {
  it('scopes lookup to workspace, connection, and provider', async () => {
    ...
    const provider = new DatabaseSecretProvider(db);
  ...
  it('dereferences optional vault credentials', async () => {
    ...
    await expect(new DatabaseSecretProvider(db, vault).getConnectionSecret({ connectionId: 'c', workspaceId: 'w', provider: 'instagram' })).resolves.toEqual({ accessToken: 'vault-token' });
