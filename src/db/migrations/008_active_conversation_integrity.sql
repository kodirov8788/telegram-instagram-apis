...
BEGIN
 ...
 IF duplicate_active_groups>0 OR invalid_connections>0 OR invalid_customers>0 THEN
  RAISE EXCEPTION 'migration 008 preflight failed: duplicate active groups=%, invalid connections=%, invalid customers=%',duplicate_active_groups,invalid_connections,invalid_customers
    USING HINT='Inspect with privileged aggregate/group queries; reconcile from authoritative records without automatic deletion or merging, then rerun.';
 END IF;
END $do$;
DO $do$ BEGIN
...
-- Keep this public signature stable; any future signature change belongs in a new migration.
...
BEGIN
 ...
 PERFORM 1 FROM public.workspace_members wm WHERE wm.workspace_id=derived_workspace AND wm.user_id=actor FOR KEY SHARE;
 ...
 LOOP
  ...
  SELECT c.* INTO conversation FROM public.conversations c WHERE c.workspace_id=derived_workspace AND c.connection_id=p_connection_id AND c.customer_id=p_customer_id AND c.channel=derived_channel AND c.status IN ('new','ai_handling','waiting_for_customer','human_attention_required','human_handling','qualified_lead');
  IF FOUND THEN RETURN conversation; END IF;
 END LOOP;
END $fn$;
