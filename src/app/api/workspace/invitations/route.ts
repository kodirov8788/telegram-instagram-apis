...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await withLiveAuthorization(req, 'members:invite', async (p, client) => {
      ...
      const inserted = await client.query(`INSERT INTO workspace_invitations (workspace_id,email,role,token_hash,invited_by,expires_at)
      ...
      if (inserted.rows[0]) {
        await AuditLogService.logEvent({
          ...
          newValue: { email: inserted.rows[0].email, role: inserted.rows[0].role },
        });
      }
      return inserted;
    });
