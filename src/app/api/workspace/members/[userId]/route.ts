...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ userId: string }> }) {
  try {
    ...
    const result = await query(`UPDATE workspace_members SET role=$1 WHERE workspace_id=$2 AND user_id=$3 AND role<>'owner'
...
export async function DELETE(req: NextRequest, ctx: { params: Promise<{ userId: string }> }) {
  try {
    ...
    const result = await query(`DELETE FROM workspace_members WHERE workspace_id=$1 AND user_id=$2 AND role<>'owner'
