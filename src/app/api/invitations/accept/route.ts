...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const invite = (await client.query(`SELECT i.* FROM workspace_invitations i JOIN users u ON u.email=i.email
      WHERE i.token_hash=$1 AND u.id=$2 AND i.accepted_at IS NULL AND i.expires_at>NOW() FOR UPDATE`, [createHash('sha256').update(body.token).digest('hex'), p.userId])).rows[0];
    ...
    await client.query(`INSERT INTO workspace_members(workspace_id,user_id,role) VALUES($1,$2,$3)
      ON CONFLICT(workspace_id,user_id) DO UPDATE SET role=EXCLUDED.role`, [invite.workspace_id,p.userId,invite.role]);
