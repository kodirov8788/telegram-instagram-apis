...
beforeEach(() => { db.mockReset(); vi.clearAllMocks(); setMockSupabaseUser({ id: 'user-1', email: 'u@test.dev' }); });
...
describe('tenant route enforcement', () => {
  it('derives workspace owner exclusively from authenticated session', async () => {
    ws.createWorkspace.mockResolvedValue({ id: wid, name: 'Acme' } as never);
  ...
  it('rejects a body-supplied owner id', async () => {
    const req = new NextRequest('https://app.test/api/workspace', { method: 'POST', headers, body: JSON.stringify({ name: 'Acme', ownerUserId: 'attacker' }) });
  ...
  it('scopes conversation mutation by workspace and hides cross-tenant ids', async () => {
    member('support_operator'); db.mockResolvedValueOnce({ rows: [] } as never);
    ...
    expect(db.mock.calls[1][0]).toContain('workspace_id = $4'); expect(db.mock.calls[1][1]).toEqual(['closed', undefined, cid, wid]);
  });
  it('prevents CSV spreadsheet formulas and scopes export to authorized tenant', async () => {
    member('read_only_analyst'); db.mockResolvedValueOnce({ rows: [{ id: cid, full_name: '=HYPERLINK("bad")', status: 'new_lead', score: 1 }] } as never);
    const res = await exportLeads(new NextRequest('https://app.test/api/leads/export', { headers })); const text = await res.text();
    expect(res.status).toBe(200); expect(text).toContain("'=HYPERLINK"); expect(db.mock.calls[1][1]).toEqual([wid]);
  });
  it('rejects an unauthorized export after live demotion', async () => {
    member('support_operator'); const res = await exportLeads(new NextRequest('https://app.test/api/leads/export', { headers }));
    expect(res.status).toBe(403); expect(db).toHaveBeenCalledTimes(1);
  });
});
